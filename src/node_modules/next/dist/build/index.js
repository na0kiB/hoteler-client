...
async function build(dir, conf = null, reactProductionProfiling = false, debugOutput = false, runLint = true) {
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            let viewPaths;
            if (viewsDir) {
                viewPaths = await nextBuildSpan.traceChild('collect-view-paths').traceAsyncFn(()=>(0, _recursiveReaddir).recursiveReadDir(viewsDir, new RegExp(`\\.(?:${config.pageExtensions.join('|')})$`))
                );
            }
            ...
            if (pageKeys.some((page)=>_constants.MIDDLEWARE_ROUTE.test(page)
            )) {
