        ...
        return new Promise((resolve, reject)=>{
            ...
            wp.on('aggregated', async ()=>{
                ...
                for (const [fileName, { accuracy , safeTime  }] of knownFiles){
                    ...
                    if (this.viewsDir && (0, _normalizePathSep).normalizePathSep(fileName).startsWith((0, _normalizePathSep).normalizePathSep(this.viewsDir))) {
                        isViewPath = true;
                        pageName = (0, _absolutePathToPage).absolutePathToPage(this.viewsDir, fileName, this.nextConfig.pageExtensions, false);
                    } else {
                        pageName = (0, _absolutePathToPage).absolutePathToPage(this.pagesDir, fileName, this.nextConfig.pageExtensions);
                    }
                    if (isViewPath) {
                    ...
                    if (regexMiddleware.test(fileName)) {
                        routedMiddleware.push(`/${(0, _path).relative(this.pagesDir, fileName).replace(/\\+/g, '/')}`.replace(/^\/+/g, '/').replace(regexMiddleware, '/'));
                        continue;
                    }
